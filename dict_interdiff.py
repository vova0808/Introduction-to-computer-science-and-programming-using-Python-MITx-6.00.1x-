# Assume you are given two dictionaries d1 and d2, each with integer keys and integer values. You are also 
# given a function f, that takes in two integers, performs an unknown operation on them, and returns a value.
# Write a function called dict_interdiff that takes in two dictionaries (d1 and d2). 
#The function will return a tuple of two dictionaries: a dictionary of the intersect of d1 and d2 and a dictionary 
#of the difference of d1 and d2, calculated as follows:
...
# the second parameter to the function. Do not implement f inside your dict_interdiff code -- assume it is defined outside.
...
def dict_interdiff(d1, d2):
    ...
    for key, value in d1.items(): #if key in d1 is als in d2, then apply function f to both values from d1 and d2
        if key in d2.keys():
            intersect[key] = f(value, d2[key])
        else:
            difference[key] = d1[key] #add key from d1 and it value to difference dictionary  
